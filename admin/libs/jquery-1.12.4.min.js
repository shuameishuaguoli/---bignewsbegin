...
// 为了书写方便我们配置ajax请求的请求头
// 设置ajax全局配置，因为每一个页面都要携带一个请求头，我们不要在每个页面中的ajax请求中都写一遍请求头的设置，ajax中有一个ajax全局的设置：
$.ajaxSetup({
  // 设置请求头
  headers: {
    //   每个页面都需要获取到存到本地的token值
    Authorization: window.localStorage.getItem('token'),
  },
});

//为了后边调用接口方便，我们要将所有的接口抽象成对象，我们使用自调用函数的格式来做
(function() {
  // 这种模式叫沙箱模式
  ...
  // 将创建好的对象需要挂载到window的DOM树上，只有挂载到了window对象上了，外边的元素才能获取到这个对象中的接口属性。
